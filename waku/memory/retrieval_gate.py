...
def should_retrieve(
    ...
    print("small model: {} 门禁判断消息是否需要检索：{}".format(small_model, message))
    ...
    try:
        ...
        if "{" not in text:   # 只有推理/被截断的回复，不是错误
            print("门禁判断消息是否需要检索：{} {} {}".format(True, message, "gate returned no JSON — failing open"))
        ...
        print("门禁判断消息是否需要检索：{} {} {}".format(bool(decision.get("retrieve")), decision.get("query", message), decision.get("reason", "")))
        ...
        print("门禁判断消息是否需要检索：{} {} {}".format(True, message, f"gate failed open ({type(exc).__name__})"))
