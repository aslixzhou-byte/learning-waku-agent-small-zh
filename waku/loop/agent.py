...
from __future__ import annotations  # 让类型注解（如 list[dict]）在旧版 Python 里也能用

from dataclasses import dataclass, field  # dataclass：自动生成 __init__/__repr__；field：配置字段默认值
from typing import Any, Callable          # Any=任意类型；Callable=可调用对象（函数）类型
import anthropic                          # Anthropic SDK：循环直接依赖的 LLM 客户端

from waku.tools.registry import ToolRegistry  # 工具注册表：schemas() 给模型看，execute() 安全执行
