...
class _FakeNotionClient:
    """notion_client.Client（>= 2.5，data-sources API）的内存版假实现。

    Pages 放在类级别，这样 dashboard 代码里创建的 store 与测试主体里创建的
    store 能看到相同的行。"""

    _pages: list[dict] = []
    _init_count = 0     # Client 构造次数——dashboard 必须只构造一个
    _query_count = 0    # data-source 查询次数——结果缓存会限制这些查询
    _query_fails = False   # 模拟成功抓取之后发生的 Notion 故障
...
def fake_notion(monkeypatch):
    ...
    # dashboard 在模块级缓存 store + 结果——测试之间要重置
...
def _isolated_home(monkeypatch, tmp_path):
    """让 collect()/memory_action() 指向一个临时的 WAKU_HOME，且不做网络预热
    （provider 为 anthropic，无 base_url）。"""
...
def test_collect_episodes_notion_outage_degrades_gracefully(monkeypatch, fake_notion, tmp_path):
    _isolated_home(monkeypatch, tmp_path)
    monkeypatch.setenv("WAKU_EPISODIC_STORE", "notion")
    monkeypatch.delenv("NOTION_TOKEN", raising=False)  # 构造函数会抛出 ValueError
    ...
    assert "facts" in data  # 其余负载仍正常渲染
...
def test_collect_builds_notion_client_once_across_refreshes(monkeypatch, fake_notion, tmp_path):
    """Issue #20：重复的 collect() 调用（dashboard 自动刷新）必须复用同一个
    Notion 客户端，并在 TTL 内提供缓存结果——而不是每次轮询都重建客户端、
    重新查询 Notion。"""
    _isolated_home(monkeypatch, tmp_path)
    monkeypatch.setenv("WAKU_EPISODIC_STORE", "notion")
    ...
    fake_notion._init_count = 0   # 忽略上面的构造逻辑
    ...
    assert fake_notion._init_count == 1   # 客户端只构造一次，随后被缓存
    assert fake_notion._query_count == 1  # 结果在 TTL 内缓存，不重新查询
...
def test_collect_refetches_after_ttl_but_reuses_client(monkeypatch, fake_notion, tmp_path):
    ...
    monkeypatch.setattr(dashboard, "_NOTION_EPISODES_TTL", 0)   # 每次轮询都已过期
    ...
    assert fake_notion._query_count == 2   # TTL 已过期 → 每次都重新抓取
    assert fake_notion._init_count == 1    # ……但客户端仍只构造一次


def test_collect_serves_stale_episodes_during_notion_outage(monkeypatch, fake_notion, tmp_path):
    """内部规则：Notion 故障必须优雅降级——当手头有缓存时，标签页会保留上次
    的完好数据而不是变为空白。"""
    _isolated_home(monkeypatch, tmp_path)
    monkeypatch.setenv("WAKU_EPISODIC_STORE", "notion")
...
def test_delete_episode_busts_the_episodes_cache(monkeypatch, fake_notion, tmp_path):
    ...
    after = dashboard.collect()   # 缓存已失效 → 重新抓取，而不是旧数据对
